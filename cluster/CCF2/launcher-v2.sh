...
# Modified by: 
# August 2020
# Script for running CCF script with multiple parameters
...
spark-submit --master $CLUSTER_TYPE $CLUSTER_OPTIONS \
	 --executor-cores $NUM_EXECUTOR_CORES --num-executors $NUM_EXECUTORS \
	 --executor-memory $EXECUTOR_MEMORY --conf spark.executor.memoryOverhead=$EXECUTOR_MEMORY_OVERHEAD \
...
if [ $retVal -ne 0 ]; then
	...
	echo "EXECUTOR_MEMORY_OVERHEAD="$EXECUTOR_MEMORY_OVERHEAD >> $LOGDIR/$RESULTS_LOG_NAME
	echo "NUM_DRIVER_CORES="$NUM_DRIVER_CORES >> $LOGDIR/$RESULTS_LOG_NAME
	echo "DRIVER_MEMORY="$DRIVER_MEMORY >> $LOGDIR/$RESULTS_LOG_NAME
	...
	# Get the yarn logs
	yarn logs -applicationId $AppId > $LOGDIR/$YARN_LOG_NAME
	# Extract the results and make the log
	echo "CLUSTER_TYPE="$CLUSTER_TYPE > $LOGDIR/$RESULTS_LOG_NAME
	echo "CLUSTER_OPTIONS="$CLUSTER_OPTIONS >> $LOGDIR/$RESULTS_LOG_NAME
	echo "NUM_EXECUTORS="$NUM_EXECUTORS >> $LOGDIR/$RESULTS_LOG_NAME
	echo "NUM_EXECUTOR_CORES="$NUM_EXECUTOR_CORES >> $LOGDIR/$RESULTS_LOG_NAME
	...
	# Generate CSV
	echo $JOB_NAME,$PYTHON_SCRIPT,$NUM_EXECUTOR_CORES,$NUM_EXECUTORS,$EXECUTOR_MEMORY,$EXECUTOR_MEMORY_OVERHEAD,$NUM_DRIVER_CORES,$DRIVER_MEMORY,$PROCESS_TIME,$PARTITIONS,$ITERATION > $LOGDIR/$CSV_LOG_NAME
fi
