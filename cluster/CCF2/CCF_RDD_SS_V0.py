...
conf = SparkConf()
conf.setAppName('pyspark-shell-CCF-RDD-SS-v0')
...
LOGGER.warn("################################")
LOGGER.warn(" Start CCF RDD with SS (v0) ")
LOGGER.warn("--------------------------------")
...
while new_pair_flag:
    ...
    reduceJob = mapJob.groupByKey().flatMap(lambda pair: CCF_Iterate_reduce_SS(pair))
